        ...
        <div class="px-4 py-5 sm:px-6 flex flex-col md:flex-row justify-between items-start md:items-center bg-gradient-to-r from-indigo-50 to-white">
            <div>
                <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate flex items-center">
                    ...
                    Quản Lý Lịch Rảnh Của Tôi
                </h2>
                <p class="mt-1 max-w-2xl text-sm text-gray-600">
                    Thiết lập các khung giờ bạn có thể học để gia sư dễ dàng đặt lịch phù hợp
                </p>
            </div>

            <div class="mt-4 md:mt-0 flex flex-col sm:flex-row gap-2">
                <a href="{{ route('student.availability.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all duration-300 transform hover:scale-105">
        ...
        <div id="content-list">
            @if($availabilities->isEmpty())
                <div class="text-center py-12 bg-gray-50 rounded-lg m-6">
                    ...
                    <h3 class="mt-4 text-lg font-medium text-gray-900">Bạn chưa có lịch rảnh nào</h3>
                    <p class="mt-2 text-base text-gray-600 max-w-md mx-auto">
                        Bắt đầu bằng cách thêm lịch rảnh cho các khung giờ bạn có thể học. Điều này giúp gia sư dễ dàng lên lịch dạy phù hợp với bạn.
                    </p>
                    <div class="mt-6 flex justify-center">
                        <a href="{{ route('student.availability.create') }}" class="inline-flex items-center px-5 py-3 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all duration-300 transform hover:scale-105">
                            <svg class="-ml-1 mr-2 h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                            </svg>
                            Thêm Lịch Rảnh
                            ...
                            @foreach($availabilities as $availability)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('student.availability.edit', $availability->id) }}" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 px-3 py-1 rounded-md inline-flex items-center mr-2 transition-colors">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                            Sửa
                                        </a>
                                        <form action="{{ route('student.availability.destroy', $availability->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('DELETE')
